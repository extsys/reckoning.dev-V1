...
...
```
See the highlighted lines for the primary changes.
...
In the novela theme, I have to update the MDX component and the article template to achieve this.
In the MDX component, I had to pass the headings parameter to the MDXRenderer.

```jsx:title=@narative/gatsby-theme-noeval/src/components/MDX/MDX.tsx
...
Notice, the use of extra CSS to disable scroll up button on small screens.
...
Notice this acts even on secrent pages, so I had to explicitly disable those in the article template:
...
These custom blocks allows one to use markdown within the block, by defining custom classes and
blocks. I love using these specially for TLDR and Update blocks.
