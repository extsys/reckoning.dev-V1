...
<ZoomImage src='https://res.cloudinary.com/sadanandsingh/image/upload/v1545501807/project/plasma_install.jpg'></ZoomImage>
...
```bash:title=/etc/systemd/network/50-wired.network
[Match]
Name=enp0s25
...
```bash:title=/etc/profile.d/kwin.sh
...
...
```bash:title=$HOME/.gnupg/gpg-agent.conf
..
enable-ssh-support
